...
if (!g.__NUSTACK_DB__) g.__NUSTACK_DB__ = buildDB();
...
export function approveStageProposal(proposalId: string, userId: string, note?: string) {
  ...
  audit({ actorType: "USER", actorId: userId, objectType: "DEAL", objectId: deal.id, eventType: "STAGE_CHANGED", beforeValue: getStage(before)?.name, afterValue: getStage(p.proposedStageId)?.name, sourceSignalIds: p.sourceSignalIds, reason: note });
